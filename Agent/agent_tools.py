...
class CodeAnalysisTools:
    ...
    def get_caller_context(self, function_name: str, commit: str, max_results: int = 3) -> List[Dict]:
        """查找函数调用位置（通常对static函数意义不大）"""
        callers = []
        try:
            # git grep -n "func_name(" commit -- (搜索函数调用)
            # 注意：排除函数定义行
            ...
            if res.returncode == 0:
                for line in res.stdout.splitlines()[:max_results * 2]:  # 多取一些，过滤后可能不够
                    # 格式: commit:path:line:content
                    parts = line.split(':', 3)
                    if len(parts) >= 4:
                        # parts[0] 是 commit hash, parts[1] 是路径
                        path = parts[1]
                        try:
                            lno = int(parts[2])
                            content = parts[3]
                        ...
                        # 跳过函数定义行（包含 'static'、'void'、'int' 等类型关键字在函数名前）
                        if re.search(r'\b(static|void|int|char|float|double|struct)\s+.*\b' + re.escape(function_name) + r'\s*\(', content):
                            continue
